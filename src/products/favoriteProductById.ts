...
export async function favoriteProductById(req: BunRequest<"/products/favorites/:id">): Promise<Response> {
  ...
  if (!id || !product_name || quantity === null) return Response.json({ message: "Product does not exists" }, { status: 404 });
  ...
  return Response.json({ message: "Product favorited" }, { status: 200 });
}
